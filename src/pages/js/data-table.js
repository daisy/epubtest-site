...
class DataTable extends LitElement {
    ...
    */
    ...
    render() {
        ...
        return html`<link rel="stylesheet" href="${this.cssUrl}"><div class="data-table">${filtersHtml} ${columnSelectorHtml}<table summary="${this.summary}" aria-live="polite" aria-colcount="${this.data.headers.length}" aria-rowcount="${rows.length}"><thead><tr>${this.data.headers.map((header, idx) => {
        ...
        */
        ...
        html`<div class="text-search"><label for="search-filter">Search</label> <input type="text" id="search-filter" name="search" @keyup="${(e) =>
                    this.setTextSearchString(
                        e.target.value
                    )}" .value="${this.textSearchString}"></div>`
        : 
        '';
        ...
        if (hasDropDownFilters) {
            filtersHtml = html`<fieldset class="filters"><legend>${this.options.filtersLabel}</legend>${Object.keys(this.options.filters).map(filterId => {
                    ...
                    return html`<div><label for="filter-${filterId}">${filter.name}</label> <select id="filter-${filterId}" @change="${(e) =>
                                ...
                                (filterOption) => html`<option value="${filterOption.value}" ?selected="${this.enabledFilters.hasOwnProperty(
                                            filterId
                                        ) &&
                                        this.enabledFilters[
                                            filterId
                                        ] ==
                                            filterOption.value}">${filterOption.name}</option>`
                            )}</select></div>`;
                })} ${textSearchHtml} <button @click="${this.clearFilters}">Reset</button></fieldset>`;
